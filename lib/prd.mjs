...
function parseRequirements(text) {
  ...
  result.push(
    ...lines(text).filter((line) =>
      /must|should|required|requirement|visual|font|layout|responsive|mobile|desktop|asset|image|hero|mascot|raster|3d|glass|cinematic|premium|texture|background|illustration|rendered|banner|overlap|clip|animated|animation|motion|reveal|intro|loop|webm|mp4|video|hyperframes|gsap/i.test(line)
    )
  );
...
function inferAssetQualityRequired(text, requirements = []) {
  const source = `${text}\n${requirements.join('\n')}`;
  return /hero|mascot|asset|image|raster|3d|glass|cinematic|premium|texture|background|illustration|rendered|animated|animation|motion|reveal|intro|loop|webm|mp4|video|hyperframes/i.test(source);
}
...
function classifyRequirement(item) {
  ...
  if (isCodeNativeVisualBehavior(item) || /button|form|flow|auth|payment|search|filter|navigation|click|interactive/i.test(item)) {
    return 'behavior';
  }
...
function makeAssetPlan({ requirements, viewport, assetQualityRequired }) {
  const assetRequirements = unique(requirements.filter(isGeneratedAssetRequirement));
  ...
  const hyperframesRequired = assets.some((asset) => asset.tool === 'hyperframes');
  const imagegenRequired = assets.some((asset) => asset.tool !== 'hyperframes');
...
- Use \`imagegen\` or production assets for complex static raster, 3D, glass, cinematic, mascot, texture, or premium image work.
