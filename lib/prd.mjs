...
function makeGuardConfig({ viewport, hardFailHints, longReference }) {
  return {
...
function makeRunbook({ configPath, summaryPath, guardConfigPath, prdChecklistPath, referencePath, url }) {
...
betterref-verify --report .betterref/report.json --guard .betterref/guard-report.json --longpage .betterref-longpage/longpage-report.json --prd ${prdChecklistPath} --out .betterref/final-verdict.json
\`\`\`
...
export async function buildPrdArtifacts(options) {
  ...
  const guardConfig = makeGuardConfig({ viewport, hardFailHints, longReference });
...
`;
  ...
  await writeFile(runbookPath, makeRunbook({ configPath, summaryPath, guardConfigPath, prdChecklistPath, referencePath, url }));
