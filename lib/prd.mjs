...
function hasContextualHuggingFaceCue(item) {
  const value = String(item || '');
  return /\bhugging\s*face\b/i.test(value) &&
    /\b(?:hunyuan(?:\s*3d)?|3d|model|generate|generation|provider|space|endpoint|glb|gltf|mesh)\b/i.test(value);
}

function isHunyuanProviderRequirement(item) {
  const value = String(item || '');
  return /\b(?:hunyuan(?:\s*3d)?|tencent\/hunyuan3d)\b/i.test(value) || hasContextualHuggingFaceCue(value);
}
...
function isThreeDModelRequirement(item) {
  ...
  return hasStrongCue ||
    hasContextualHuggingFaceCue(value) ||
...
# Hugging Face path, requires HF_TOKEN:
...
# Tencent Cloud path, requires TENCENTCLOUD_SECRET_ID and TENCENTCLOUD_SECRET_KEY:
