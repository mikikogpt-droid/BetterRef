...
\`\`\`bash
# Read ${assetPlanPath}; for each pending item, use imagegen or a production asset.
# Save the generated/source asset to its targetPath, wire it into the app, capture fresh browser evidence,
# then mark that asset item pass only after betterref-guard validates scale and sharpness.
\`\`\`
...
betterref-verify --report .betterref/report.json --guard .betterref/guard-report.json --longpage .betterref-longpage/longpage-report.json --prd ${prdChecklistPath} --asset-plan ${assetPlanPath} --require guard,prd,longpage,assetplan --out .betterref/final-verdict.json --html .betterref/final-verdict.html --bundle .betterref/evidence-bundle.json
\`\`\`
