...
function matchesExpectation(actual, expected = {}) {
  ...
  const assetPrompts = asArray(actual.assetPlan?.prompts).join('\n');
  const assetRoles = asArray(actual.assetPlan?.roles).join('\n');
  ...
  for (const forbiddenPrompt of asArray(expected.assetPlanForbiddenPromptIncludes)) {
    if (assetPrompts.includes(String(forbiddenPrompt))) {
      mismatches.push(`expected asset plan prompts not to include ${forbiddenPrompt}`);
    }
  }
  for (const requiredPrompt of asArray(expected.assetPlanRequiredPromptIncludes)) {
    if (!assetPrompts.includes(String(requiredPrompt))) {
      mismatches.push(`expected asset plan prompts to include ${requiredPrompt}`);
    }
  }
  for (const forbiddenRole of asArray(expected.assetPlanForbiddenRoleIncludes)) {
    if (assetRoles.includes(String(forbiddenRole))) {
      mismatches.push(`expected asset plan roles not to include ${forbiddenRole}`);
    }
  }
  for (const requiredRole of asArray(expected.assetPlanRequiredRoleIncludes)) {
    if (!assetRoles.includes(String(requiredRole))) {
      mismatches.push(`expected asset plan roles to include ${requiredRole}`);
    }
  }
...
async function summarizeAssetPlan(assetPlanPath) {
  ...
  return {
    ...
    prompts: assets.map((asset) => asset.prompt || '').filter(Boolean),
