...
const TENCENT_GLOBAL_ENDPOINT = 'hunyuan.intl.tencentcloudapi.com';
const TENCENT_GLOBAL_REGION = 'ap-singapore';
...
function providerTypes({ provider }) {
  const normalized = String(provider || 'tencent').trim().toLowerCase();
  if (normalized !== 'tencent') {
    throw new BetterRef3DError(`Tencent Cloud is the only supported Hunyuan3D provider. Use --provider tencent, not ${provider || normalized}.`);
  }
  return ['tencent'];
}

function rejectLegacyProviderOptions({ space, endpoint, customUrl }) {
  const legacy = [
    space ? '--space' : null,
  ...
  if (legacy.length > 0) {
    throw new BetterRef3DError(`Legacy provider adapter options are no longer supported: ${legacy.join(', ')}. Use Tencent Cloud options.`);
  }
}

function tencentEditionConfig(edition) {
  const normalized = String(edition || 'rapid').trim().toLowerCase();
  if (normalized === 'pro') {
    return {
      edition: 'pro',
...
function normalizeTencentEndpoint(endpoint) {
  return String(endpoint || TENCENT_GLOBAL_ENDPOINT)
    .trim()
    .replace(/^https?:\/\//i, '')
    .replace(/\/+$/g, '')
    .toLowerCase();
...
function assertTencentGlobalEndpoint(endpoint) {
  const normalized = normalizeTencentEndpoint(endpoint);
  if (normalized !== TENCENT_GLOBAL_ENDPOINT) {
    throw new BetterRef3DError(
      `Signed Tencent HY 3D Global API is required for BetterRef 3D. Use --tencent-endpoint ${TENCENT_GLOBAL_ENDPOINT}; do not use ${endpoint || normalized}.`
    );
  }
  return TENCENT_GLOBAL_ENDPOINT;
}
...
export async function makeHunyuanRequest({
  ...
  rejectLegacyProviderOptions({ space, endpoint, customUrl });
  ...
  const globalEndpoint = assertTencentGlobalEndpoint(tencentEndpoint);
  request.tencentCloud = {
    endpoint: globalEndpoint,
    region: tencentRegion || TENCENT_GLOBAL_REGION,
  ...
  request.auth = {
    ...
    signedApi: true,
    signatureVersion: 'TC3-HMAC-SHA256',
    workstationCredentialSources: [
      'env:TENCENTCLOUD_SECRET_ID',
      'env:TENCENTCLOUD_SECRET_KEY',
      'blender-scene-official-api'
    ],
    missingEnvGuidance: 'If Tencent env vars are absent on this workstation, read Blender MCP scene official API SecretId/SecretKey and submit a signed Tencent HY 3D Global API request directly; do not fall back to Blender MCP legacy/domestic Hunyuan generation.',
...
function isHunyuanAsset(asset) {
  ...
  const implementation = String(asset.implementation || '').toLowerCase();
  const tool = String(asset.tool || '').toLowerCase();
  return provider === 'hunyuan' || provider === 'tencent' || tool.includes('hunyuan') || implementation.includes('hunyuan') || implementation.includes('tencent');
}

function hasTencentOnlySource(asset) {
  return isHunyuanAsset(asset);
}

function hasRequestMetadata(source, asset) {
  if (!source || typeof source !== 'object' || Array.isArray(source)) return false;
  const hasProvider = asArray(source.providers).length > 0 ||
    Boolean(source.provider || source.tencentCloud);
  if (!hasProvider) return false;
  const items = metadataItems(source);
  return items.some((item) => {
    if (!metadataMatchesAsset(item, asset)) return false;
    if (!isTencentMetadata(source, item)) return true;
    return hasSignedTencentGlobalRequest(source);
  });
}
...
function hasSignedTencentGlobalRequest(source) {
  const tencentCloud = source?.tencentCloud || {};
  const endpointOk = normalizeTencentEndpoint(tencentCloud.endpoint) === TENCENT_GLOBAL_ENDPOINT;
  const regionOk = !tencentCloud.region || tencentCloud.region === TENCENT_GLOBAL_REGION;
  const actionOk =
    (tencentCloud.submitAction === 'SubmitHunyuanTo3DProJob' && tencentCloud.queryAction === 'QueryHunyuanTo3DProJob') ||
    (tencentCloud.submitAction === 'SubmitHunyuanTo3DRapidJob' && tencentCloud.queryAction === 'QueryHunyuanTo3DRapidJob');
  const auth = source?.auth || {};
  return Boolean(endpointOk && regionOk && actionOk && auth.signedApi === true && auth.legacyMcpDisallowed === true);
}
...
function blockingReason(code, asset) {
  ...
  if (code === 'non_tencent_model_source') return `${asset.id} is not sourced from Tencent Hunyuan3D.`;
...
export async function verify3D({
  ...
  for (const asset of assets) {
    ...
    const tencentOnlySourcePresent = hasTencentOnlySource(asset);
    const hunyuanMetadataRequired = true;
    ...
    const signedTencentGlobalApiPresent = !hunyuanMetadataRequired ||
      requestMetadataSources.some((source) => hasRequestMetadata(source, asset));
    const requestMetadataPresent = signedTencentGlobalApiPresent;
    ...
    if (!tencentOnlySourcePresent) {
      assetFailures.push(failure('non_tencent_model_source', asset, `3D asset ${asset.id} must originate from Tencent Hunyuan3D.`));
    }
    ...
    checkedAssets.push({
      ...
      tencentOnlySourcePresent,
      hunyuanMetadataRequired,
      signedTencentGlobalApiPresent,
