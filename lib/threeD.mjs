...
function providerTypes({ provider, endpoint, customUrl }) {
  ...
  if (!['space', 'endpoint', 'both', 'custom'].includes(normalized)) {
    throw new BetterRef3DError(`Unknown Hunyuan provider: ${provider}. Use space, endpoint, both, or custom.`);
  }
...
function metadataMatchesAsset(item, asset) {
  ...
  const itemId = String(item.id || item.assetId || item.modelId || '');
  if (assetId && itemId && assetId === itemId) return true;
  const assetTarget = pathKey(asset.targetPath);
  const itemTarget = pathKey(item.targetPath || item.modelPath || item.outputPath || item.generatedPath);
...
function hasRequestMetadata(source, asset) {
  ...
  const hasProvider = asArray(source.providers).length > 0 ||
    Boolean(source.provider || source.huggingFace || source.space || source.endpoint || source.customUrl);
  if (!hasProvider) return false;
  const items = metadataItems(source);
  return items.length === 0 || items.some((item) => metadataMatchesAsset(item, asset));
}

function hasResponseMetadata(source, asset) {
  if (!source || typeof source !== 'object' || Array.isArray(source)) return false;
  const items = metadataItems(source);
  const candidates = items.length > 0 ? items.filter((item) => metadataMatchesAsset(item, asset)) : [source];
  return candidates.some((item) => {
    const status = String(item.status || item.state || item.result || '').trim();
    const statusOk = !status || isPassStatus(status);
