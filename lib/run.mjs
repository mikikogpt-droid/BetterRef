...
function renderNextActions(result) {
  ...
  if (result.artifacts.threeDPlanPath) {
    ...
    lines.push('');
    lines.push('Generate the model through Tencent Cloud Hunyuan3D, record request metadata, then attach mesh and turntable evidence before browser verification.');
    lines.push('');
    ...
    lines.push(`betterref-3d --make-hunyuan-request --plan ${result.artifacts.threeDPlanPath} --out ${result.artifacts.threeDOut || path.dirname(result.artifacts.threeDPlanPath)} --provider tencent --tencent-region ap-guangzhou --tencent-edition pro --tencent-model 3.1 --result-format GLB --enable-pbr true --face-count 50000 --json`);
  ...
  if (threeDRequired) {
    ...
    if (!handoff.passed) {
      blockers.push({
        ...
        message: 'A Hunyuan 3D model is required. Generate it through Tencent Cloud, record response metadata, attach mesh/render evidence, and rerun 3D verification before browser verification can pass.',
        nextAction: [
          ...
          `betterref-3d --make-hunyuan-request --plan ${paths.threeDPlanPath} --out ${paths.threeDOut} --provider tencent --tencent-region ap-guangzhou --tencent-edition pro --tencent-model 3.1 --result-format GLB --enable-pbr true --face-count 50000 --json`,
