...
function getReportMode(report) {
  ...
  if (report.mode && typeof report.mode === 'object') {
    return report.mode.name || report.mode.mode || report.mode.type;
  }
  return undefined;
}
...
function checkReport({ report, config, browserEvidence, hardFails }) {
  ...
  const longReference =
    ...
    reportMode === 'full_page_scroll_reference' ||
    Boolean(report.mode && typeof report.mode === 'object' && report.mode.longPage) ||
  ...
  if (reportMode !== expectedMode) {
    hardFails.push({
      code: 'long_reference_missing_scroll_mode',
      message: `Long-page references must use ${expectedMode}, not ${reportMode || 'unknown'}.`
    });
  }
...
function renderedAssetArea(asset) {
  const width = numericValue(asset.renderedWidth, asset.cssWidth, asset.clientWidth, asset.width);
  const height = numericValue(asset.renderedHeight, asset.cssHeight, asset.clientHeight, asset.height);
  if (!width || !height) {
    return undefined;
  }
  return width * height;
...
function countRenderedProductionAssets({ report, config, browserEvidence }) {
  const minArea = numericValue(config.minRenderedAssetArea, 1024);
  const assets = [
    ...asArray(report.renderedAssets),
    ...asArray(report.assets?.rendered),
    ...asArray(browserEvidence.images),
  ...
  for (const asset of assets) {
    const src = browserImageSource(asset);
    if (!src || /^(data|blob|about):/i.test(String(src))) {
      continue;
    }
    const area = renderedAssetArea(asset);
    if (minArea !== undefined && area !== undefined && area < minArea) {
      continue;
    }
    seen.add(String(src));
  }
...
function checkRenderedAssetCoverage({ report, config, browserEvidence, hardFails, summary }) {
  const minRenderedAssets = numericValue(config.minRenderedAssets);
  const renderedProductionAssets = countRenderedProductionAssets({ report, config, browserEvidence });
  summary.renderedProductionAssets = renderedProductionAssets;

  if (minRenderedAssets === undefined) {
    return;
  }

  if (renderedProductionAssets < minRenderedAssets) {
    hardFails.push({
      code: 'browser_missing_rendered_assets',
      message: `Browser evidence shows ${renderedProductionAssets} rendered production assets, below required minimum ${minRenderedAssets}.`,
      required: minRenderedAssets,
      actual: renderedProductionAssets
    });
  }
}
...
function checkBrowserEvidence({ browserEvidence, config, hardFails }) {
  ...
  const networkErrors = [
    ...asArray(browserEvidence.network?.errors),
    ...asArray(browserEvidence.networkErrors)
  ];
  if (networkErrors.length > 0 && config.failOnNetworkErrors !== false) {
    hardFails.push({
      code: 'browser_network_error_present',
      message: 'Browser evidence contains failed network responses or requests.',
      count: networkErrors.length,
      sample: networkErrors.slice(0, 3)
    });
  }
...
export async function runGuard(options) {
  ...
  const summary = {
    ...
    renderedAssetsChecked:
      ...
      asArray(browserEvidence.videos).length +
      asArray(browserEvidence.media?.rendered).length +
  ...
  checkRenderedAssetCoverage({ report, config, browserEvidence, hardFails, warnings, summary });
