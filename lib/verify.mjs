...
function assetPlanVerdict(plan) {
  if (!plan) {
    return {
      present: false,
      passed: true,
  ...
  const pending = assets
    ...
    .map((asset, index) => ({
      ...
      requirement: asset.requirement || asset.role || asset.targetPath || ''
    }));
  ...
  return {
    present: true,
    passed: pending.length === 0 && !emptyRequiredPlan,
    hardFailPresent: pending.length > 0 || emptyRequiredPlan,
    imagegenRequired,
    total: assets.length,
    passedCount: assets.length - pending.length,
    pending,
    emptyRequiredPlan
  };
}
...
export async function verifyFinal(options) {
  ...
  const assetPlan = assetPlanVerdict(assetPlanReport);
