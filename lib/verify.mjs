...
import { validateAgentMergeEvidence } from './agents.mjs';
...
function parseRequiredEvidence(value) {
  const aliases = new Map([
    ['all', ['guard', 'prd', 'longpage', 'assetplan', 'browser', '3d', 'agents']],
    ['guard', ['guard']],
    ['prd', ['prd']],
    ['checklist', ['prd']],
    ['longpage', ['longpage']],
...
async function validateThreeDAssetProof(asset, { required, projectDir } = {}) {
  ...
  if (threeDAssetNeedsHunyuanMetadata(asset, required) && asset.signedTencentGlobalApiPresent !== true) {
    reasons.push(`3D asset ${id} is missing signed Tencent HY 3D Global request proof`);
  }
...
function requiredEvidenceVerdict(required, parts) {
  ...
  if (required.includes('agents') && !parts.agentEvidence.present) {
    missing.push('agents');
  }
  return {
    required,
...
function summarizeAgentEvidence(agentEvidence) {
  return {
    present: agentEvidence.present,
    passed: agentEvidence.passed,
    hardFailPresent: agentEvidence.hardFailPresent,
    runtimeMode: agentEvidence.runtimeMode || null,
    selectedAgents: agentEvidence.selectedAgents || [],
    reportCount: agentEvidence.reportCount || 0,
    blockingReasons: agentEvidence.blockingReasons || []
  };
}
...
async function buildEvidenceBundle(report, options) {
  const artifactInputs = [
    ...
    ['agent-merge', options.agentMergePath],
  ...
  return {
    ...
    verdict: {
      ...
      agentEvidencePassed: report.agentEvidence.passed
    },
    ...
    agentEvidence: summarizeAgentEvidence(report.agentEvidence),
...
export async function verifyFinal(options) {
  ...
  const agentEvidence = await validateAgentMergeEvidence(options.agentMergePath);
  ...
  if (!agentEvidence.passed) {
    for (const reason of agentEvidence.blockingReasons) {
      blockingReasons.push(reason);
    }
    if (agentEvidence.blockingReasons.length === 0) {
      blockingReasons.push('agent evidence did not pass');
    }
  }
  if (!assetPlan.passed) {
  ...
  const finalReport = {
    ...
    inputs: {
      ...
      agentMerge: options.agentMergePath ? path.resolve(options.agentMergePath) : null,
