...
async function assetPlanVerdict(plan, options = {}) {
  ...
  const hyperframesRequired = Boolean(plan.hyperframesRequired || assets.some(isHyperframesAsset));
  const imagegenRequired = Boolean(plan.imagegenRequired || assets.some((asset) => !isHyperframesAsset(asset)));
  const assetPlanRequired = imagegenRequired || hyperframesRequired || assets.length > 0;
  const emptyRequiredPlan = assetPlanRequired && assets.length === 0;
  const passedCount = assets.filter((asset, index) => {
...
function threeDVerdict(report, options = {}) {
  ...
  if (!passed && (!hasRequiredPassFields || !hasValidPassFields)) {
...
export async function verifyFinal(options) {
  ...
  const threeD = threeDVerdict(threeDReport, { supplied: Boolean(options.threeDPath) });
