...
export async function captureChrome(options) {
  ...
  try {
    ...
    const fullPageScreenshotPath = fullPage ? path.join(outDir, 'chrome-full-page.png') : null;
    ...
    if (fullPage) {
      const fullPageScreenshot = await client.send('Page.captureScreenshot', {
        format: 'png',
        fromSurface: true,
        captureBeyondViewport: true
      });
      if (!fullPageScreenshot.data) {
        throw new BetterRefChromeError('Chrome did not return full-page screenshot data.');
      }
      await writeFile(fullPageScreenshotPath, Buffer.from(fullPageScreenshot.data, 'base64'));
    }

    if (sectionScreenshots) {
      const sectionDir = path.join(outDir, 'sections');
      await mkdir(sectionDir, { recursive: true });
      for (const element of domBoxes.elements) {
        const box = element.absoluteBox || element.boundingBox;
        ...
        const sectionPath = path.join(sectionDir, `${slugName(element.name)}.png`);
        ...
        if (!sectionScreenshot.data) {
          throw new BetterRefChromeError(`Chrome did not return section screenshot data for ${element.name}.`);
        }
        await writeFile(sectionPath, Buffer.from(sectionScreenshot.data, 'base64'));
    ...
    await writeFile(browserEvidencePath, `${JSON.stringify({
      ...
      source: {
        tool: 'betterref-chrome',
        url: domBoxes.viewport?.url || target.url,
        title: domBoxes.viewport?.title || target.title
      },
      viewport: domBoxes.viewport,
      page: domBoxes.page || {},
      fonts: domBoxes.fonts || {},
      images: domBoxes.images || [],
      videos: domBoxes.videos || [],
      assets: domBoxes.assets || { rendered: domBoxes.images || [] },
