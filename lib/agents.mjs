...
function selectTeams(task, requestedTeams = []) {
...
export async function planAgentRun({
  ...
  const teams = selectTeams(task, selectedTeams);
  ...
  return {
    ...
    runtimeMode,
    packet,
    artifacts: { outDir: resolvedOut, supervisorPacketPath }
  };
}
...
export async function runAgentWorkflow({
  ...
  const plan = await planAgentRun({ task, outDir, projectDir, assetId, runtimeMode, inputs, selectedTeams });
  const { packet } = plan;
