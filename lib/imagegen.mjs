...
function pendingAssets(plan) {
  return asArray(plan.assets).filter((asset) =>
    !isHyperframesAsset(asset) &&
    !isThreeDModelAsset(asset) &&
    !isPassStatus(asset.status || asset.state || asset.result)
  );
}
...
export async function analyzeImagegenStatus(options) {
  ...
  const assets = asArray(plan.assets).filter((asset) => !isHyperframesAsset(asset) && !isThreeDModelAsset(asset));
  const items = [];
  const counts = {};
