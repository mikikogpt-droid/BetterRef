...
async function requireFilePath(value, baseDir, label) {
  const rawPath = typeof value === 'string'
    ? value
    : value?.path || value?.file || value?.screenshotPath || value?.src;
  if (!rawPath) {
    throw new BetterRefChromeBridgeError(`Chrome handoff is missing ${label} screenshot path.`);
  }
  ...
  try {
    ...
    throw new BetterRefChromeBridgeError(`Chrome handoff ${label} screenshot does not exist: ${resolved}`);
  }
  if (!info.isFile()) {
    throw new BetterRefChromeBridgeError(`Chrome handoff ${label} screenshot is not a file: ${resolved}`);
  }
  return resolved;
}
...
async function normalizeScreenshots(input, baseDir) {
  ...
  const fullPageInput =
    ...
    input.fullPageScreenshotPath ||
    input.fullPagePath;

  const screenshotPath = await requireFilePath(viewportInput, baseDir, 'viewport');
  const fullPageScreenshotPath = fullPageInput
    ? await requireFilePath(fullPageInput, baseDir, 'full-page')
    : null;

  const sectionScreenshotPaths = [];
  for (const [index, section] of asArray(screenshots.sections || input.sectionScreenshotPaths).entries()) {
    const sectionPath = await requireFilePath(section, baseDir, `section ${index + 1}`);
    sectionScreenshotPaths.push({
      name: section?.name || section?.label || section?.selector || `section-${index + 1}`,
      selector: section?.selector || null,
      path: sectionPath,
      clip: section?.clip || section?.rect || section?.boundingBox || null
    });
  }
  ...
  return {
    ...
    screenshots: {
      ...
      sections: sectionScreenshotPaths
    }
  };
}
...
export async function bridgeChromeHandoff(options) {
  ...
  const screenshots = await normalizeScreenshots(input, baseDir);
  const source = {
  ...
  const browserEvidence = {
    ...
    videos: asArray(input.videos || input.media?.videos),
    media: input.media || {},
    screenshotPath: screenshots.screenshotPath,
    viewportScreenshotPath: screenshots.viewportScreenshotPath,
    fullPageScreenshotPath: screenshots.fullPageScreenshotPath,
    sectionScreenshotPaths: screenshots.sectionScreenshotPaths,
  ...
  if (options.configOutPath) {
    const config = await buildRegionConfig({
      ...
      viewport,
      mergePath: options.mergeConfigPath,
