...
export async function compareLongPage(options) {
  ...
  for (const item of sectionItems(evidence)) {
    ...
    try {
      ...
      sections.push({
        ...
        passed: false,
        score: 0,
        referenceSlicePath: null,
        actualPath: item.path || null,
        reportPath: null,
        diffPath: null,
        clip: item.clip || null,
        dimensionDrift: null,
  ...
  const sectionHardFailHints = sections
    .filter((section) => section.error)
    .map((section) => `section ${section.name} failed: ${section.error}`);
  const report = {
    ...
    verdict: {
      ...
      hardFailHints: sections.length === 0 ? ['missing section screenshots'] : sectionHardFailHints
    },
    artifacts: {
