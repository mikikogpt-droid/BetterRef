...
Exit code `0` means the configured thresholds passed. Exit code `1` means revise/fail after verification. Exit code `2` means invalid usage or config. For `betterref-run`, exit code `3` means the run is blocked by required external evidence such as imagegen output, HyperFrames CLI evidence, or browser evidence.
